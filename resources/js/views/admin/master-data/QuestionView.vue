  ...
  <div class="p-8 max-w-[1440px] w-full mx-auto">
    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-sm border border-outline-variant/10 overflow-hidden fade-in-delay">
      ...
      <div v-else-if="questions.length === 0" class="p-12 text-center">
        ...
        <p class="text-body-base text-on-surface-variant mt-3">Belum ada pertanyaan untuk indikator ini.</p>
        <button
          @click="openAddModal"
            ...
            >
              ...
              <td class="px-6 py-5">
                <div class="flex items-center justify-center gap-1">
                  <button @click="openViewModal(q)" class="p-2 text-on-surface-variant hover:text-primary hover:bg-primary/10 rounded-lg transition-all" title="Lihat Detail">
                    <span class="material-symbols-outlined text-[18px]">visibility</span>
                  </button>
                  <button @click="handleEdit(q)" class="p-2 text-on-surface-variant hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all" title="Edit">
                    <span class="material-symbols-outlined text-[18px]">edit</span>
                  </button>
                  <button @click="handleDelete(q)" class="p-2 text-on-surface-variant hover:text-error hover:bg-error/10 rounded-lg transition-all" title="Hapus">
        ...
        <div class="relative bg-surface-container-lowest rounded-2xl shadow-xl w-full max-w-lg z-10 modal-content">
          ...
          <div class="flex items-center justify-between p-6 border-b border-outline-variant/10">
            <div class="flex items-center gap-3">
              ...
              <div>
                <h3 class="font-title-md text-title-md text-on-surface">Detail Pertanyaan</h3>
                <p class="text-body-sm text-on-surface-variant">Informasi lengkap pertanyaan</p>
              </div>
            </div>
            <button
          ...
          <!-- Modal Body -->
          <div class="p-6 space-y-5" v-if="viewingQuestion">
            <!-- Teks Pertanyaan -->
            <div>
              <label class="block font-label-caps text-label-caps text-on-surface-variant uppercase tracking-wider mb-2">Teks Pertanyaan</label>
              <textarea
                :value="viewingQuestion.questionText || viewingQuestion.text"
                rows="3"
                disabled
            ...
            <div class="pt-4 border-t border-outline-variant/20">
              <div class="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p class="font-medium text-on-surface">{{ formatDate(viewingQuestion.createdAt) }}</p>
                </div>
                <div>
                  <span class="text-secondary">Diupdate:</span>
                  <p class="font-medium text-on-surface">{{ formatDate(viewingQuestion.updatedAt) }}</p>
                </div>
              </div>
            </div>
          </div>
